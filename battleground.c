...
{ // Player joins team
	...
	if( bg->creation_tick == 0 )
		bg->creation_tick = last_tick; // Creation Tick = First member joined.
...
{ // Single Player leaves team
	...
	switch( flag )
	{
	case 3: sprintf(output, "[Battleground] : %s ha sido sacado de la sala por AFK...", sd->status.name); break;
	case 2: sprintf(output, "[Battleground] : %s ha sido sacado de la sala por un reporte AFK...", sd->status.name); break;
	case 1: sprintf(output, "[Battleground] : %s ha abandonado la sala...", sd->status.name); break;
	case 0: sprintf(output, "[Battleground] : %s se ha ido del campo de batalla...", sd->status.name); break;
	}
	clif_bg_message(bg, 0, "[Battleground]", output, strlen(output) + 1);
...
void bg_team_rewards(int bg_id, int nameid, int amount, int kafrapoints, int quest_id, const char *var, int add_value, int bg_arena, int bg_result)
{
	...
	{ // BG Reward Rates
		amount = amount * battle_config.bg_reward_rates / 100;
		if(battle_config.bg_kafrapoints)
	...
	for( j = 0; j < MAX_BG_MEMBERS; j++ )
	{
		...
		if( kafrapoints > 0  && battle_config.bg_kafrapoints)
